...
//                return RedirectToAction("History"); // Redirect to their history page on success
//            }
//            return View(model);
//        }

//        // --- VIEW CLAIM HISTORY (FOR BOTH ROLES) ---
//        [HttpGet]
...
//            {
//                if (!string.IsNullOrEmpty(status)) claimsQuery = claimsQuery.Where(c => c.Status == status);
//                if (claimId.HasValue) claimsQuery = claimsQuery.Where(c => c.ClaimId == claimId.Value);
//                if (submissionDate.HasValue) claimsQuery = claimsQuery.Where(c => c.SubmissionDate.Date == submissionDate.Value.Date);
//            }
...
//            // --- FIX IS HERE ---
//            // Add a null check for the related User object.
//            if (claim.User == null)
//            {
//                // This handles cases where data might be inconsistent.
//                return BadRequest("The claim is not associated with a valid user.");
//            }
...
//                    return View(model);
//                }
//                // Validation logic from your API
...
// This alias is a best practice to prevent naming conflicts.
using SecurityClaim = System.Security.Claims.Claim;
...
namespace ClaimsMVC.Controllers
{
    ...
    public class ClaimsController : Controller
    {
        ...
        // THIS IS THE ONLY HISTORY METHOD - IT HANDLES EVERYTHING
        [HttpGet]
        public async Task<IActionResult> History(string? status, int? claimId, DateTime? submissionDate)
        {
            // FIX: Use the fully qualified name to resolve the ambiguity.
            IQueryable<ClaimsMVC.Data.Models.Claim> claimsQuery = _context.Claims.Include(c => c.User);
            ...
            // If the user is ONLY an employee, filter the list to only their claims.
